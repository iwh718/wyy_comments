...
class GetWangYiYunInfo
{
    ...
    private function httpGet($url,$header)
    {
        ...
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 2);        // 设置等待时间
    ...
    private function saveComment($data,$songName)
    {
        ...
        for ($i=0;$i<$hotCommentsLength;$i++){
             $this->list[]  = array('name' => $songName, 
                'comments' => $data['hotComments'][$i]['content'],
                'time' => date("Y-m-d H:i:s",$data['hotComments'][$i]['time'] / 1000),
                'user' => $data['hotComments'][$i]['user']['nickname'],
                'like' =>  $data['hotComments'][$i]['likedCount']);
        }
